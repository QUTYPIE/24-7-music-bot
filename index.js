...
const web = new WebhookClient({ url: 'https://discord.com/api/webhooks/1026145868883177543/NfyCtYzPgVSpAWTOncMt0Rgpj90X1UhGx3DiN_SaBg_O9QK0p9x9xPfTljJlaZqbD7U5' }); 
...
client.on('interactionCreate', async interaction => {
    ...
    if(funny === 'first') {
        ...
        .setDescription("● To get help on a specific command type `⚙️ made by  ᴠᴀʏᴜ | Q U T Y P I E <3 , ₦ł₲H₮ ₣ɄⱤɎ`!")
        .setColor("#F806F4")
        .setFooter("⚙️ made by  ᴠᴀʏᴜ | Q U T Y P I E <3 , ₦ł₲H₮ ₣ɄⱤɎ", ee.avatara)
        .setTimestamp()
    ...
    if(funny === 'second') {
        ...
        .setDescription("● To get help on a specific command type `Galaxy Music help <command>`!")
        .setColor('#F806F4')
        .setFooter("⚙️ made by  ᴠᴀʏᴜ | Q U T Y P I E <3 , ₦ł₲H₮ ₣ɄⱤɎ", ee.avatara)
        .setTimestamp()
...
client.on('interactionCreate', async interaction => {
    ...
    if(player === undefined) {
        if(interaction.channel.name === 'Galaxy -requets') {
            setTimeout(() => {
...
client.on('guildCreate', async (guild) => {
    try {
        ...
        .addField("**Members**", `${guild.memberCount} + <@947891831516065834>`, true)
    try { embed.addField("**Region**", guild.region, true) } catch {/** */}
    
    client.channels.cache.get("1000734885788536872").send({embeds: [embed]})
  ...
  try {
    ...
    const texts = "Hi, This Is GALAXY MUSIC DUOS  If You Need Support Related Me Join My Support Server. Support Server Link Is Here https://discord.gg/Rqdx38Gdfn"
